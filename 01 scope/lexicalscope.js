...
//用。它首先从最内部的作用域，也就是 fn2(..) 函数的作用域气泡开始查找。引擎无法在
//这里找到 a ，因此会去上一级到所嵌套的 fn1(..) 的作用域中继续查找。在这里找到了 a ，
//因此引擎使用了这个引用。对 b 来讲也是一样的。而对 c 来说，引擎在 fn2(..) 中就找到
//了它。
//如果 a 、 c 都存在于 fn2(..) 和 fn1(..) 的内部， console.log(..) 就可以直接使用 fn2(..)
//中的变量，而无需到外面的 fn1(..) 中查找。
